...
export default function Page8Outcomes({ tr, formData, updateData }) {
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        <QuestionBlock number="20" label={tr.q20} required>
          <div className="space-y-2 mb-4">
            {OUTCOMES.map(outcome => (
              <PercentInput
                key={outcome.key}
                label={outcome.label}
                value={pcts[outcome.key]}
                onChange={setPct(outcome.key)}
              />
            ))}
          </div>
