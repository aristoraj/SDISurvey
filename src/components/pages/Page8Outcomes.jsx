...
export default function Page8Outcomes({ tr, formData, updateData, errors }) {
  const e = errors || {};
        ...
        <QuestionBlock number="20" label={tr.q20} required error={e.outcomePercentages}>
          <div className="space-y-2 mb-4">
            {OUTCOMES.map(outcome => (
              <PercentInput key={outcome.key} label={outcome.label} value={pcts[outcome.key]} onChange={setPct(outcome.key)} />
            ))}
          </div>
